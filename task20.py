...
def scrabble():
    sum = 0
    word_rand = input('Введите слово: ')
    word_up = word_rand.upper()
    for i in range(len(word_up)):
        key = word_up[i]
        if key in eng_dict:
            sum +=eng_alphabet(word_up[i])
        else: sum +=ru_alphabet(word_up[i])
    print(sum)
